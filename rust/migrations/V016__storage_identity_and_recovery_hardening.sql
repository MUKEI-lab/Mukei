-- Forward-only hardening for Universal Storage identity, import recovery, and
-- operation-journal evidence. V015 is frozen; all post-V015 safety invariants
-- live here so databases that already applied V015 can upgrade normally.
...
-- Once an import row exists, its identity and authorization target are immutable.
...
-- Progress is durable evidence. Idempotent replay may repeat a byte count, but
...
-- Terminal imports are stable facts and cannot be revived by stale workers.
...
-- Scope primary identity, ownership, and root binding are immutable after creation.
...
-- A node may claim scope_root only when it is the exact root declared by scope.
...
-- Immutable object bytes require immutable identity metadata. Integrity lifecycle
-- fields remain mutable so corruption/quarantine can still be recorded.
...
-- Journal node evidence must remain within its declared scope.
...
-- Import-backed journal evidence must retain the import transaction's scope.
...
-- Journal primary identity is immutable. Terminal evidence becomes an audit fact.
