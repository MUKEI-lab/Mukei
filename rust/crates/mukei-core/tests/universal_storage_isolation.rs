...
fn insert_workspace(connection: &Connection, suffix: &str) {
    connection.execute(
        ...
        params![format!("scope-{suffix}"), format!("workspace-{suffix}"),
                format!("chat-{suffix}"), format!("root-{suffix}")],
    ).unwrap();
    connection.execute(
    ...
    connection.execute(
        ...
        params![format!("uploads-{suffix}"), format!("scope-{suffix}"), format!("root-{suffix}")],
    ).unwrap();
}
